...
def main():
    ...
    m3 = MotionParam(jnt_speed=100, lin_speed=10, pose_speed=100, overlap=0, acctime=0.4, dacctime=1, posture=2)
    rb.motionparam(m1)                                                                                              
	...
	# 2.2 get block grasping coordinates from client
	    ...
	    if ddz+335 < -135:
	 	rb.relline(dz = -135+detect_obstacle)
	    
	    else:
		if block_orientation == 'short':
		    print "block orientation is short"
	            rb.relline(dz = ddz+200+245+10+detect_obstacle)
		else:
		    print "block orientation is long"
		    rb.relline(dz = ddz+200+245+10+10+detect_obstacle)
	...
	while True:
		...
		w_orientation = data.get("w_orientation")
		if w_orientation == "long":
		    gripper_mode = "place_long"
		    z_offset = -30
	            place_z_offset = -60
		else:
		    gripper_mode = "place_short"
		    z_offset = -20
		    place_z_offset = -50
	        w_x, w_y, w_d6 = waypoint_target
	        rb.relline(dx = w_x + 5, dy = w_y, dz = -60, drz = w_d6)
	        #rb.reljntmove(dj6 = -w_d6)
		rb.relline(dz = z_offset)
	        send_gripper_and_wait(server,"close")
	...
	print "Place gripper mode : ", gripper_mode
	send_gripper_and_wait(server,'open',gripper_mode, speed=50)
	time.sleep(0.2)
