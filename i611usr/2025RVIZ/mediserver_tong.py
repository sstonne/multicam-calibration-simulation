...
class MedicineHandoverServer:
    ...
    def __init__(self, rb, log_filename="medicine_handover_server_log.txt"):
    ...
    def move_to_target(self, x, y, z, rz, state="moving"):
        try:
            ...
            target = self.rb.relline(dx=x, dy=y, dz=z, drz=rz)
            self.rb.asyncm(2)
            time.sleep(1)
            return True
        except Exception, e:
    ...
    def pick_and_give(self, target_pos):
        rb, server = self.rb, self.server
        HANDOVER_JOINT = [18.38, -57.58, -44.65, -181.57, 76.37, -13.72]

        x, y, z, rz = target_pos
        print "[SERVER] Pick and give at (%s, %s, %s)" % (x, y, z)
    ...
    def wait_for_grip_confirmation(self, timeout=3.0):
        ...
        while time.time() - start < timeout:
            try:
                rlist, _, _ = select.select([self.server.conn], [], [], 0.1)
                if not rlist:
                    continue
    ...
    def remove_obstacle(self, target_pos):
        rb, server = self.rb, self.server
        x, y, z, rz = target_pos
        print "[SERVER] Removing obstacle at (%s, %s, %s)" % (x, y, z)
        ...
        rb.relline(dz=150)
        target_joint = self.DISCARD_JOINTS[self.discard_index]
        rb.motionparam(self.DEFAULT_PARAM)
        rb.move(Joint(*target_joint))
        server.send_command({"action": "open_gripper", "data": {"open_mm": 85}})
    ...
    def main(self):
        ...
        while True:
            ...
            if action == "set_symptom":
                ...
                print "[SERVER] Symptom='%s' → Required='%" %(symptom.encode('utf-8') if isinstance(symptom, unicode) else symptom,
    required_class.encode('utf-8') if isinstance(required_class, unicode) else required_class)
                self.move_to_camera_view()
                time.sleep(2.0)
                client_ready = True
                server.send_command({"action": "start_detect"})
