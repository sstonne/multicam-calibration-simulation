...
class MedicineHandoverServer:
    ...
    def pick_and_give(self, target_pos):
        ...
        self.send_command({"action": "open_gripper", "data": {"open_mm": 85}})
        self.move_to_target(x, y, z + 100, rz)
        self.rb.relline(dz=-100)
        self.server.send_command({"action": "open_gripper", "data": {"open_mm": 20, "force": 0}})
        time.sleep(1)
        self.rb.relline(dz=50)
        time.sleep(1)
        self.server.send_command({"action": "open_gripper", "data": {"open_mm": 0, "force": 0}})
        time.sleep(1)
        self.server.send_command({"action": "check_grip_status"})
        ...
        self.rb.relline(dz=150)
        self.rb.motionparam(self.DEFAULT_PARAM)
        self.rb.asyncm(2)
        self.rb.move(Joint(*HANDOVER_JOINT))
        time.sleep(0.5)
        self.server.send_command({"action": "open_gripper", "data": {"open_mm": 85}})
        time.sleep(0.5)
        ...
        if self.has_discarded:
            ...
            self.has_discarded = False
            self.server.send_command({"action": "stop"})
    ...
    def main(self):
        ...
        self.server.send_command({"action": "medicine_start"})
        data  = comm(self.server)
        action = data.get("action")

        if action == "medicine_start":
            client_ready = True
        ...
        while True:
            ...
            if action == "start_detect" and status == "move_recover":
                # move recover 동작 실행
                self.rb.motionparam(self.DEFAULT_PARAM)
                self.rb.relline(dz=30, dx=10)
                time.sleep(0.5)
                print "[SERVER] Recovery done."

            elif action == "start_detect" and status == "True":
                position = data.get("position")
                sub_action = data.get("goal")
                # 약 잡으러 이동 실행       
                position[2] += 215
                ...
                if sub_action == "pick_and_give":
                    success = self.pick_and_give(position)

                    if success == "grip_fail":
                        print('grip fail..')
                        continue

                elif sub_action == "remove":
                    success = self.remove_obstacle(position)
