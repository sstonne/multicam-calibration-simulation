...
DEFAULT_PARAM = MotionParam(jnt_speed=70, lin_speed=50, overlap=50)
#버림 위치 
DISCARD_JOINTS= [
...
def pick_and_give(rb, target_pos):
    ...
    #server.send_command({"action": "close_with_force",
    #"data": {"force_ratio": 0.15}})
    server.send_command({"action": "open_gripper",
...
def restore_discarded_items(rb):
    ClOSE_JOINT = [-7.60, -23.78,-105.58,-180.00,  50.62,  -7.59]
    """버려둔 약들을 다시 담는 동작"""
	...
	server.send_command({"action": "open_gripper", "data": {"open_mm": 0, "force": 0}})
        ...
        # 카메라 뷰로 돌아와 놓기
        #move_to_camera_view(rb)  #이거는 추후에 놓을 상자 조인트 넣어라
...
def remove_obstacle(rb, target_pos):
    ...
    print "[SERVER] Re-checking grip status..."
    #server.send_command({"action": "close_with_force",
    #"data": {"force_ratio": 0.1}})
    server.send_command({"action": "open_gripper",
    "data": {"open_mm": 10, "force":0}})
    time.sleep(1)
    server.send_command({"action": "check_grip_status"})
    grip_ok = wait_for_grip_confirmation(timeout=3.0)
