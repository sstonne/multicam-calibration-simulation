...
| --- | --- | --- | --- |
| cam0 | 039422061216 | `False` | 테이블 고정, 교체된 D415 |
...
**두 보드가 같은 DICT_4X4를 쓴다.** 현재 로봇 보드는 OpenCV 기본값인 ID 0부터로
인쇄됐다고 추정해 코드에서 ID 0~16으로 설정했다. 이 추정이 맞으면 테이블 보드(ID 5~42)와
**5~16번 12개가 겹친다.**
...
없다.** 현재 `capture/board_config.py`의 ID 0은 기본값에 근거한 추정이며, 실물 보드만
찍어 확인하기 전까지 확정값이 아니다.
...
보내지 않는다. `--dataset-root`의 기본값은 `<repo>/datasets`이며, 세션의 RGB, depth,
...
`record_dataset.py`의 기본 세션 데이터는 `<repo>/datasets/`에 저장하며 Git 아카이브
