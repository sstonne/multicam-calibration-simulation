...
> 이 문서는 촬영 전 준비 기록이다. 완료된 재촬영의 최신 분석은 [2026-09-05~06 재촬영 정식 평가](real_recapture_evaluation_20260910.md)를 따른다. 2026-09-04 데이터는 분석 입력이 아닌 백업이다.
...
| --- | --- | --- | --- |
| cam0 | 039422061216 | `False` | 테이블 고정 |
...
규격은 `capture/board_config.py`에 단일 정의로 둔다. 2026-09-10에 교체된
로봇 보드를 사용자 규격과 cam1 실물 영상으로 확인했다.

| | 로봇 보드 | 테이블 보드 |
| --- | --- | --- |
| 위치 | 로봇 팔 끝(플랜지) | 광학 테이블 |
| OpenCV 칸 수 (x×y) | **9 × 6** | 11 × 7 |
| 실험실 표기 (세로×가로) | 6 × 9 | 7 × 11 |
| checker | **25 mm** | 25 mm |
| marker | **18 mm** | 18 mm |
| dictionary | DICT_4X4_250 | DICT_4X4_250 |
| 체커 영역 크기 (여백 제외) | **225 × 150 mm** | 275 × 175 mm |
| 내부 코너 수 | **40** | 60 |
| 마커 수 | **27** | 38 |
| 마커 ID | **90~116** | 5~42 |
| legacy_pattern | **True** | False |

짝수 행인 새 로봇 보드는 `setLegacyPattern(True)`가 필요하다. 같은 실물
프레임에서 legacy=True는 코너 40/40개, False는 0/40개였다. ID나 칸 수만
수정하면 ArUco 마커가 보여도 ChArUco 코너를 못 찾을 수 있다.

`--resume`은 보드 ID뿐 아니라 칸 수, 실제 길이, 딕셔너리, 인쇄 패턴도
비교한다. 보드를 교체한 뒤에는 새 세션으로 시작한다.

기존 cam0 보정 파일은 연결되지 않은 카메라 314522062542용이었다. 연결된
039422061216과 일치하는 `rb-calibration-marker-experiment/intrinsics/cam0.npz`
(2026-09-07 ChArUco 보정, 0.216px)를 적용했다. 이전 파일과 출처 기록은
`intrinsics/backup_before_board_update_20260910/`에 보관한다. cam1/cam3는 기존 파일을 사용한다.

## 3. 마커 ID와 실물 검증

로봇 ID 90~116과 테이블 ID 5~42는 서로 겹치지 않는다. 검사 도구는 현재
설정 대역의 검출/미검출 ID와, 설정에 없는 ID를 나누어 표시한다. 일부 마커가
가려질 수 있으므로 검출된 최소 ID만으로 시작 ID를 결정하지 않는다.

```bash
python capture/check_board_ids.py --live --serial 319522062138
python capture/record_dataset.py
```

실제 레코더로 cam0/cam1/cam3 기동, 자세 서버 12352 연결, 진단 촬영 1건의
이미지·자세 저장을 확인했다. 당시 자세에서는 cam1만 로봇 보드를 보고 있어
코너 수가 0/40/0이었다. 해당 진단 레코드는 `gate_fail`이며 캘리브레이션에
적합한 촬영으로 간주하면 안 된다. 보드가 필요한 카메라들에 보이는 자세로 촬영한다.

아래 §4~§6의 시뮬레이션 수치는 교체 전 7×5 / 17mm 보드를 사용한 과거
실험 결과다. 새 보드의 정확도 예측으로 직접 해석하지 않는다.
